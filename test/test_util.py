...
class TestUtil(unittest.TestCase):
    ...
    def test_image_encoder_b64(self):
        ...
        non_existing_file_name = 'non-existing-file.png'
        ...
        with self.assertRaises(FileNotFoundError):
            image_encoder_b64(
                image_file_or_path=non_existing_file_name,
            )

        with self.assertRaises(TypeError) as error:
            image_encoder_b64(True)
        check_error_message(self, error, type_err_msg)
    ...
    @patch('weaviate.util.get_valid_uuid')
    def test_generate_local_beacon(self, mock_get_valid_uuid):
        """
        Test the `generate_local_beacon` function.
        """

        mock_get_valid_uuid.side_effect = lambda uuid: uuid

        # error messages
        type_err_msg = lambda dt: (
            f"'class_name' must be of type str. Given type: {dt}"
        )

        # invalid calls
        with self.assertRaises(TypeError) as error:
            generate_local_beacon('test', None)
        check_error_message(self, error, type_err_msg(NoneType))

        with self.assertRaises(TypeError) as error:
            generate_local_beacon('test', 123)
        check_error_message(self, error, type_err_msg(int))
        
        # valid calls
        beacon = generate_local_beacon("fcf33178-1b5d-5174-b2e7-04a2129dd35b", 'Test')
        self.assertEqual(
            beacon,
            {"beacon" :"weaviate://localhost/Test/fcf33178-1b5d-5174-b2e7-04a2129dd35b"},
        )
    ...
    def test_get_valid_uuid(self):
        ...
        # invalid calls
        type_err_msg = lambda dt: f"'uuid' must be of type str or uuid.UUID. Given type: {dt}"
        value_err_msg = "Not valid 'uuid' or 'uuid' can not be extracted from value"

        ## wrong type
        with self.assertRaises(TypeError) as error:
            get_valid_uuid(12342)
        check_error_message(self, error, type_err_msg(int))

        # wrong UUID format
        with self.assertRaises(ValueError) as error:
            get_valid_uuid("some-UUID")
        check_error_message(self, error, value_err_msg)

        # valid calls
        result = get_valid_uuid(UUID("28f3f61b-b524-45e0-9bbe-2c1550bf73d2"))
        self.assertEqual(result, "28f3f61bb52445e09bbe2c1550bf73d2")

        result = get_valid_uuid(UUID("28f3f61bb52445e09bbe2c1550bf73d2"))
        self.assertEqual(result, "28f3f61bb52445e09bbe2c1550bf73d2")
    ...
    def test_generate_uuid5(self, mock_uuid):
        """
        Test the `generate_uuid5` function.
        """

        from uuid import NAMESPACE_DNS
        mock_uuid.NAMESPACE_DNS = NAMESPACE_DNS

        result = generate_uuid5('TestID!', 'Test!')
        self.assertIsInstance(result, str)
        mock_uuid.uuid5.assert_called_with(NAMESPACE_DNS, 'TestID!Test!')
    ...
    def test_capitalize_first_letter(self):
        ...
        result = capitalize_first_letter('t')
        ...
        result = capitalize_first_letter('testTest_test')
        self.assertEqual(result, 'TestTest_test')
